...
<script lang="ts">
	...
	const classes = $derived(
		buildClass({
			classes: [
				`radio radio-${theme} appearance-none focus:ring-0 focus:ring-offset-0 bg-transparent border border-frame-400 dark:border-frame-700 [&:not(:checked)]:bg-frame-100 dark:[&:not(:checked)]:bg-frame-800 form-checkbox`,
				!theme && 'text-frame-500 dark:text-frame-700',
