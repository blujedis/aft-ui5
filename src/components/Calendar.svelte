<script lang="ts">
	...
	function createEvents() {
		...
		for (let i = 0; i < 7; ++i) {
			...
			days[i] = day.getFullYear() + '-' + pad(day.getMonth() + 1) + '-' + pad(day.getDate());
		}
	...
	onMount(() => {
		const EventCalendar = (window as any).EventCalendar;
		eCal = new EventCalendar(cal, options);
	});
</script>
...
<svelte:head>
	<link
		rel="stylesheet"
		href="https://cdn.jsdelivr.net/npm/@event-calendar/build@3.4.0/event-calendar.min.css"
	/>
	<script
		src="https://cdn.jsdelivr.net/npm/@event-calendar/build@3.4.0/event-calendar.min.js"
	></script>
</svelte:head>

<div bind:this={cal}></div>
